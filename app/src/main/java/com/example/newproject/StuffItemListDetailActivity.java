...
public class StuffItemListDetailActivity extends AppCompatActivity {
    ...
    public void findstuff() {
        ...
        databaseReference = database.getReference(user.getUid()).child("stuff").child(itemkey);
    ...
    public void getitemkey(final String type) {
        ...
        databaseReference.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                StuffItemInfo stuffItemInfo = snapshot.getValue(StuffItemInfo.class);
                s_key[0] = stuffItemInfo.getKey();    //stuff db에 저장되어있는 키값
                if (type == "clear") {            //거래완료
                    itemclear();
    ...
    public void itemclear() {    //두개의 db에 모두 접근하여 state정보 수정
        ...
        user = FirebaseAuth.getInstance().getCurrentUser();
        database = FirebaseDatabase.getInstance("https://newproject-ab6cb-write.firebaseio.com/");
        database.getReference(user.getUid()).child("stuff").child(itemkey).child("state").setValue("close");    //수정
    }
    ...
    public void delete() {
        user = FirebaseAuth.getInstance().getCurrentUser();
        database = FirebaseDatabase.getInstance("https://newproject-ab6cb-write.firebaseio.com/");
        database.getReference(user.getUid()).child("stuff").child(itemkey).removeValue();
    }
    ...
    public void update(String item_key) {
        ...
        StuffItemInfo stuffItemInfo = new StuffItemInfo(user.getUid(), localname, address, localurl, textname, stuff, datelimit, extratext, "open", item_key);
        database = FirebaseDatabase.getInstance("https://newproject-ab6cb-write.firebaseio.com/");
        database.getReference(user.getUid()).child("stuff").child(itemkey).setValue(stuffItemInfo);

        database = FirebaseDatabase.getInstance("https://newproject-ab6cb-stuff.firebaseio.com/");
        database.getReference(first).child(second).child(third).child(item_key).setValue(stuffItemInfo);
    }
